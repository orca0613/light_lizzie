...
class KatagoSettingDialog(QDialog):
  def __init__(self, parent=None):
    ...
    self.komi_input = LabeledInputWidget(
      "덤", str(self.setting_json[KOMI_KEY]), True, self
    )
    layout.addWidget(self.komi_input)

    # 비짓 카운트 레이아웃

    self.vc_input = LabeledInputWidget(
      "비짓 카운트", str(self.setting_json[MAX_VISIT_KEY]), True, self
    )
    layout.addWidget(self.vc_input)

    # 분석 시간 레이아웃

    self.as_input = LabeledInputWidget(
      "분석 시간", str(self.setting_json[MAX_ANALYSIS_TIME_KEY]), True, self
    )
    layout.addWidget(self.as_input)

    # 갱신 주기 레이아웃

    self.uc_input = LabeledInputWidget(
      "갱신 주기", str(self.setting_json[UPDATE_CYCLE_KEY]), True, self
    )
