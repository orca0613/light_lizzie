...
from helper import (
  close_window,
  get_range_by_move_number,
  get_range_by_score,
  get_range_by_winrate,
  normalize_bluespot_score,
  normalize_delta_score,
  normalize_delta_winrate,
)
...
class AnalysisWindow(QMainWindow):
  def __init__(self, color: str):
    ...
    self.delta_winrate = 0
    self.delta_score = 0
    self.delta_complexity = 0
    self.bluespot_ratio = 0
    ...
    # 기본값: 흑돌 (검은 원에 흰 테두리)
    self.stone_icon.setStyleSheet(
      f"background-color: {stone_color}; border: 1px solid {border_color}; border-radius: 12px;"
    )
  ...
  def update_data(
    ...
    bluespot_ratio: float,
    count: int,
  ):
    self.delta_winrate = delta_winrate
    self.delta_score = delta_score
    self.delta_complexity = delta_complexity
    self.bluespot_ratio = bluespot_ratio
  ...
  def update_analysis_data(
    self, move_number: int, winrate: float, score: float, complexity: float
  ):
    if self.df is None:
      return
    ...
    dw = sliced_df["dw"].mean()
    ds = sliced_df["ds"].mean()
    dc = sliced_df["dc"].mean()
    br = sliced_df["bs"].mean() * 100

    self.update_data(float(dw), float(ds), float(dc), float(br), count)
  ...
  def closeEvent(self, event: QCloseEvent):
    key = (
      BLACK_ANALYSIS_WINDOW_KEY if self.color == "black" else WHITE_ANALYSIS_WINDOW_KEY
    )
    close_window(key)
    event.accept()
