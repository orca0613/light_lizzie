...
class WinrateBar(QMainWindow):
  ...
  def paintEvent(self, event):
    ...
    base_prime_color = black_primary_color if self.is_white_first else white_primary_color
    base_sub_color = black_sub_color if self.is_white_first else white_sub_color
    top_prime_color = white_primary_color if self.is_white_first else black_primary_color
    ...
    if top_score > 0:
      ...
      painter.drawText(top_rect.adjusted(0, 0, -10, 0), Qt.AlignRight | Qt.AlignVCenter, score_str)
    else:
      painter.setPen(pen_color)
    ...
    # 왼쪽 끝 
    left_percent_str = f"{white_winrate:.1f}%" if self.is_white_first else f"{self.black_winrate:.1f}%" 
    painter.drawText(QRectF(0, text_y, w, text_h), Qt.AlignLeft | Qt.AlignTop, left_percent_str)
    
    # 오른쪽 끝 
    right_percent_str = f"{self.black_winrate:.1f}%" if self.is_white_first else f"{white_winrate:.1f}%"
    painter.drawText(QRectF(0, text_y, w, text_h), Qt.AlignRight | Qt.AlignTop, right_percent_str)
