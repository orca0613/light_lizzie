...
class MenuController:
  def __init__(self, main_window):
    ...
    self.katago_setting_dialog.update_katago_setting_signal.connect(
      self._set_katago_setting
    )
    self.display_setting_dialog.update_display_setting_signal.connect(
      self._set_display_setting
    )
    self.player_setting_dialog.change_player_signal.connect(self._set_player_setting)
    self.window_setting_dialog.update_window_setting_signal.connect(
      self._set_window_setting
    )

  # 카타고 관련 경로 설정
  def set_katago_path(self):
    katago_path, _ = QFileDialog.getOpenFileName(
      self.main_window, "카타고 실행 파일 선택", "", "Exec (*.*)"
    )
    if not katago_path:
      return
  ...
  def set_model_path(self):
    model_path, _ = QFileDialog.getOpenFileName(
      self.main_window, "가중치 모델 선택", "", "Model (*.*)"
    )
    if not model_path:
      return
  ...
  def set_config_path(self):
    config_path, _ = QFileDialog.getOpenFileName(
      self.main_window, "컨피그 파일 선택", "", "Config (*.*)"
    )
    if not config_path:
      return
