...
from windows.main_board_window import MainBoardWindow
from windows.marker_board_window import MarkerBoardWindow
from windows.opening_data_board_window import OpeningDataBoardWindow
from windows.prediction_board_window import PredictionBoardWindow
...
class MainController:
  def __init__(self):
    ...
    self.main_board_window.undo_signal.connect(self._undo)
    self.main_board_window.played_pos.connect(self._play_move)
    self.main_board_window.update_katago_path_signal.connect(self._check_katago_path)
    self.main_board_window.update_katago_setting_signal.connect(
      self._update_katago_setting
    )
    self.main_board_window.update_display_setting_signal.connect(
      self._update_display_setting
    )
    self.main_board_window.update_player_name_signal.connect(self._update_player)
    self.main_board_window.update_window_setting_signal.connect(
      self._update_window_setting
    )
    self.main_board_window.update_position_signal.connect(self._update_position)
    self.main_board_window.closed_signal.connect(self._close_windows)
    ...
    if self.engine:
      ...
      self.engine.set_rule_and_komi(
        self.katago_setting_json[RULE_KEY], self.katago_setting_json[KOMI_KEY]
      )
      self._start_analyze()
  ...
  def _set_engine(self):
    ...
    if not katago:
      return QMessageBox.warning(
        self.main_board_window,
        "카타고 경로 오류",
        "카타고 실행 파일 경로를 설정해 주세요!",
      )
    if not model:
      return QMessageBox.warning(
        self.main_board_window,
  ...
  def _play_move(self, y: int, x: int):
    ...
    self.marker_board_window.set_last_move(y, x)

    if not self.engine:
      QMessageBox.warning(self.main_board_window, "에러", "엔진 설정이 필요합니다.")
      return
    ...
    color_str = "white" if self.main_board_window.move_number % 2 == 0 else "black"
  ...
  def process_latest_analysis(self):
    ...
    is_white_turn = bool(self.main_board_window.move_number % 2)
    ...
    info_blocks = get_info_blocks(line)
    if not info_blocks:
      return
    best_data = get_best_data(info_blocks)
    winrate = best_data["winrate"]
    score = best_data["scoreLead"]
    complexity = best_data["scoreStdev"]
    visits = best_data["visits"]
    cur_time = time.time()
    if (cur_time - self.analysis_time > max_analysis_time) or visits > max_visit:
      self._stop_analyze()
      analysis_window = (
        self.white_analysis_window if is_white_turn else self.black_analysis_window
      )
    ...
    if is_white_turn:
      self.white_analysis_window.update_analysis_data(
        self.main_board_window.move_number, winrate, score, complexity
      )
    else:
      self.black_analysis_window.update_analysis_data(
        self.main_board_window.move_number, winrate, score, complexity
      )
  ...
  def _undo(self):
    self.moves.pop()
    if not self.moves:
      self.marker_board_window.set_last_move(-1, -1)
    else:
      y, x = self.moves[-1]
      self.marker_board_window.set_last_move(y, x)
  ...
  def _check_katago_path(self):
    ...
    QMessageBox.information(
      self.main_board_window, "", "변경 사항은 재부팅 후 반영됩니다."
    )
  ...
  def _update_display_setting(self):
    self.main_board_window.update_board_display()
    self.marker_board_window.update_board_display()
    self.opening_data_board_window.update_board_display()
  ...
  def _update_analysis_data(self, analysis_data: Dict, is_white_turn: bool):
    ...
    if is_white_turn:
      self.white_analysis_window.update_data(
        delta_winrate, delta_score, bluespot_ratio, count
      )
    else:
      self.black_analysis_window.update_data(
        delta_winrate, delta_score, bluespot_ratio, count
      )

  def _update_position(self, ps: str):
    self.opening_data_board_window.update_position(
      ps, self.main_board_window.move_number
    )

  def _close_windows(self):
    close_window("all")
    self.opening_data_board_window.close()
